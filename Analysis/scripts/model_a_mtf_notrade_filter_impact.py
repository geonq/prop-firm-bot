"""No-trade filter impact analysis for Model A MTF confluence export (2019–2026).

Same ablation logic as model_a_notrade_filter_impact.py but adapted for the
updated TV export column names (Trade number, Net PnL USD, Long-/Short- Typ prefix).

Usage:
    python3 Analysis/scripts/model_a_mtf_notrade_filter_impact.py
"""
...
DEEP_CSV = (
    ROOT
    / "TVExports"
    / "geonq_Model_A_KeyOpen_OTE_V0_CME_MINI_MNQ1!_2026-05-26_0adc5.csv"
)
...
def load_parents(path: Path) -> pd.DataFrame:
    ...
    assert not df.empty, "CSV loaded empty"

    df["Datum und Uhrzeit"] = pd.to_datetime(df["Datum und Uhrzeit"])

    # New export uses "Long-Einstieg" / "Short-Einstieg" — .str.contains still matches
    entries = df[df["Typ"].str.contains("Einstieg", na=False)].copy()
    exits   = df[df["Typ"].str.contains("Ausstieg", na=False)].copy()

    # Column name changed: "Trade number" (new) vs "Trade #" (old)
    trade_col = "Trade number" if "Trade number" in df.columns else "Trade #"
    pnl_col   = "Net PnL USD"  if "Net PnL USD"  in df.columns else "G&V netto USD"

    entry_idx = entries.set_index(trade_col)[
    ...
    out = exits.merge(entry_idx, left_on=trade_col, right_index=True, how="left")
    ...
    parents["entry_dow"]    = parents["entry_ts"].dt.dayofweek
...
def main() -> None:
    ...
    m_news_hour  = parents["entry_date"].isin(HIGH_IMPACT_DATES) & (parents["entry_hour"] == 8)
    ...
    print("=== No-Trade Filter Impact — MTF export (sorted by P&L improvement) ===\n")
    hdr = (
        f"  {'Rule':<44}  {'n_rem':>6}  {'%rem':>6}  "
    ...
    for _, r in results.iterrows():
        print(
            f"  {r['rule']:<44}  {int(r['n_removed']):>6}  {r['pct_removed']:>6.1%}  "
    ...
    print("\n=== Confluence score distribution (total sc=) ===")
    sig = parents["side"].astype(str)
    sc = sig.str.extract(r"sc=(\d+)").rename(columns={0: "score"})
    a  = sig.str.extract(r"A=(\d+)").rename(columns={0: "A"})
    b  = sig.str.extract(r"B=(\d+)").rename(columns={0: "B"})
    sc["score"] = pd.to_numeric(sc["score"], errors="coerce")
    sc["A"] = pd.to_numeric(a["A"], errors="coerce")
    sc["B"] = pd.to_numeric(b["B"], errors="coerce")
    sc["net_pnl"]   = parents["net_pnl"].values
    sc["is_winner"] = parents["is_winner"].values

    def _grp(df: pd.DataFrame, col: str) -> None:
        g = df.dropna(subset=[col]).astype({col: int})
        g = g.groupby(col).agg(
            n=("net_pnl", "size"), net=("net_pnl", "sum"),
            avg=("net_pnl", "mean"), WR=("is_winner", "mean"),
        ).reset_index()
        print(f"  {col:>5}  {'n':>5}  {'net':>10}  {'avg':>8}  {'WR':>6}")
        for _, r in g.iterrows():
            print(f"  {int(r[col]):>5}  {int(r['n']):>5}  ${r['net']:>9,.0f}  ${r['avg']:>+7.0f}  {r['WR']:.1%}")

    if sc["score"].notna().any():
        _grp(sc, "score")
    else:
        print("  No sc= tags found in Signal column.")

    if sc["A"].notna().any():
        print("\n=== Layer A (OTE TF stack 0-14) distribution ===")
        _grp(sc, "A")
    if sc["B"].notna().any():
        print("\n=== Layer B (SMC 0-7) distribution ===")
        _grp(sc, "B")

    if sc["A"].notna().any() and sc["B"].notna().any():
        print("\n=== Cross-tab: rows = Layer A, cols = Layer B (n / net) ===")
        valid = sc.dropna(subset=["A", "B"]).astype({"A": int, "B": int})
        pivot_n   = valid.pivot_table(index="A", columns="B", values="net_pnl", aggfunc="size", fill_value=0)
        pivot_net = valid.pivot_table(index="A", columns="B", values="net_pnl", aggfunc="sum", fill_value=0.0)
        print("  n trades:")
        print(pivot_n.to_string())
        print("\n  net P&L $:")
        print(pivot_net.round(0).to_string())
