"""ONE-SHOT holdout evaluation of the round-4 "alt" candidate, all four firms.

Frozen config (chosen 2026-07-18 from the full-scope 2015-2025, 18-fold
search after manual override of the automated Stage-B pick, which was
rejected as fragile/single-firm-admissible with a broken DSR read):
  or=5min, first_candle entry, or_opposite stop, 4R target, 1-tick slippage,
  120min time stop, NO vwap_trail. Risk $400/trade (round-3's finding that
  aggressive risk beats $200; $400 chosen as broadly strong across all four
  firms in the pooled pre-holdout analysis, not just chasing TopStep's peak
  at $800). params_hash=8afbe6259cab2dd2.

Same protocol as the 2026-07-17 holdout run: single pass, all four firms
scored on the IDENTICAL frozen trade list (no re-tuning), immutable record,
sentinel-locked immediately after. This is the FIRST and ONLY holdout look
for this params_hash -- the sentinel refuses any second run.

Holdout window unchanged: >= 2025-07-01 (HOLDOUT_START), same as every prior
round today. Uses the EXTENDED 2015-2026 parquet (holdout window identical
either way; extended file used for consistency with today's data source).
"""
...
OUT_DIR = ROOT / "Analysis" / "output" / "orb" / "full_scope"
...
def main() -> None:
    ...
    assert params_h == "8afbe6259cab2dd2", f"hash mismatch: {params_h}"
    ...
    record = {
        ...
        "params": {
            ...
            "vwap_trail_after_r": FROZEN.vwap_trail_after_r,
            "time_stop_minutes": FROZEN.time_stop_minutes,
        },
        "protocol_note": "round-4 full-scope holdout, single pass, all four firm-variants on identical frozen trade list",
        "data_source": "extended 2015-2026 parquet (holdout window identical to original)",
